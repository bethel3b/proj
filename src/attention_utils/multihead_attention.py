...
from src.attention_utils.scaled_dot_product_attention import scaled_dot_product_attention
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask: torch.Tensor | None = None):
        ...
        attention_output = scaled_dot_product_attention(query, key, value, is_causal=self.is_causal, attn_mask=attn_mask, dropout=dropout)

        # Reshape for output - (batch_size, q_len, d_model)
        attention_output = attention_output.transpose(1, 2).contiguous().view(batch_size, q_len, self.d_model)
...
if __name__ == "__main__":
    ...
    multihead_attention = MultiHeadAttention(d_model=d_dim, n_heads=1, dropout=0.0, is_causal=is_causal)
