...
class EncoderDecoderTransformer(nn.Module):
    def __init__(self, vocab_size: int, d_model: int, d_ff: int, n_heads: int, n_layers: int, max_seq_len: int, dropout: float = 0.0):
        ...
        self.encoder_embed_layer = PositionalEncoder(vocab_size,d_model, max_seq_len, dropout)
        self.decoder_embed_layer = PositionalEncoder(vocab_size,d_model, max_seq_len, dropout)
    ...
    def forward(self, source_tokens, target_tokens, source_attention_mask=None, target_attention_mask=None):
        ...
        source_padding_mask = (~source_attention_mask.bool()).unsqueeze(1).unsqueeze(2) \
            if source_attention_mask is not None else None

        target_padding_mask = (~target_attention_mask.bool()).unsqueeze(1).unsqueeze(2) \
            if target_attention_mask is not None else None
        ...
        encoder_output = self.encoder(encoder_input=source_embed, encoder_padding_mask=source_padding_mask)

        # Decode the target tokens
        decoder_output = self.decoder(encoder_output=encoder_output, decoder_input=target_embed, encoder_padding_mask=source_padding_mask, decoder_padding_mask=target_padding_mask)
...
if __name__ == "__main__":
    vocab_size, d_model, d_ff, n_heads, n_layers, max_seq_len, dropout = 10, 5, 10, 1, 2, 12, 0.1
    # create a encoder decoder transformer
    encoder_decoder_transformer = EncoderDecoderTransformer(vocab_size=vocab_size, d_model=d_model, d_ff=d_ff, n_heads=n_heads, n_layers=n_layers, max_seq_len=max_seq_len, dropout=dropout)
    ...
    # forward pass
    output = encoder_decoder_transformer(source_tokens=source_tokens, target_tokens=target_tokens)
    print(output.shape)
    print(output)